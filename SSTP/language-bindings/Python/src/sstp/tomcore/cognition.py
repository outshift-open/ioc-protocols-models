...
def _build_listener_belief(
    belief_store: Optional[Any],
    agent_id: str,
    concept_id: str,
    use_case: str,
) -> Dict[str, Any]:
    """Build a listener_belief dict from AgentBeliefStore for the ie_utterance_judge."""
...
class AgentTOM:
    """Per-agent Theory-of-Mind state — prediction loop and drift signals."""
    ...
    def belief(self) -> Dict[str, Any]:
        """Return peer EMA mean as confidence proxy."""
        if self._peer_ema:
            confidence = round(sum(self._peer_ema.values()) / len(self._peer_ema), 4)
        else:
            confidence = 0.5
    ...
    def assess_utterance(self, utterance: str = "", task_goal: str = "", speaker: str = "",
        ...
        """Grounding check: structural topic check first, then LLM argument-engagement check."""
        ...
        if has_concepts:
            ...
            if not verified:
                # Topic mismatch — no need to invoke LLM judge
                return {
                    "aligned": False,
            ...
            if listener_prior_utterance is not None:
                ...
                if confidence_before is not None:
                    llm_payload["confidence_before"] = confidence_before
                llm_result = self._llm.complete_json("ie_utterance_judge", llm_payload)
                llm_contingency = max(0.0, min(1.0, float(llm_result.get("contingency_score", 1.0))))
                merged_score = round(min(score, llm_contingency), 4)
                merged_aligned = verified and bool(llm_result.get("aligned", True)) and merged_score >= 0.5
                return {
                    "aligned": merged_aligned,
                    "alignment_score": round(max(0.0, min(1.0, float(llm_result.get("alignment_score", score)))), 4),
                    "disagreement_score": round(1.0 - merged_score, 4),
                    "derailed": not merged_aligned,
                    "derailment_cause": llm_result.get("derailment_cause") or None,
                    "grounding_failure": not merged_aligned,
                    "contingency_score": merged_score,
                    "ambiguous": bool(llm_result.get("ambiguous", False)),
                    "ambiguity_score": round(max(0.0, min(1.0, float(llm_result.get("ambiguity_score", 0.0)))), 4),
                    "judge_confidence": round(max(0.0, min(1.0, float(llm_result.get("judge_confidence", 0.85)))), 4),
                    "critique": f"structural+llm: {llm_result.get('critique', '')}",
                }
            # No prior utterance — structural result is sufficient
            return {
                "aligned": verified,
                "alignment_score": score,
                "disagreement_score": round(1.0 - score, 4),
                ...
                "ambiguity_score": 0.0,
                "judge_confidence": 1.0,
                "critique": "structural_grounding_check:no_prior",
            }
    ...
    def detect_ambiguity(self, utterance: str = "", task_goal: str = "") -> Dict[str, Any]:
        ...
        return {
            ...
            "ambiguous_spans": list(result.get("ambiguous_spans", [])),
            "plausible_interpretations": list(result.get("plausible_interpretations", [])),
        }

    def peer_alignment(self, peer_id: str = "", task_goal: str = "",
                       peer_belief_override: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        peer = peer_belief_override or self._peer_beliefs.get(peer_id, {})
        ema = self._peer_ema.get(peer_id, 0.5)
        alignment_score = round(
            (float(peer.get("confidence", 0.5)) * 0.5 + ema * 0.5), 4
        )
        return {
            "task_goal": task_goal,
            "alignment_score": alignment_score,
            "disagreement_score": round(1.0 - alignment_score, 4),
            "attribution_accuracy": round(float(peer.get("confidence", 0.5)), 4),
            "coherence_rationale": peer.get("context_summary", ""),
            "role_objectives": {peer_id: peer.get("objective", "")},
        }
    ...
    def update_peer(
        ...
        # argument_strength from the tom_belief_update LLM result — replaces the
        # assess_task_alignment stub that always returned 0.5.
        alignment_score = round(
