...
```python
class SNPSkill:
    def open_convergence(self, concept_id: str, group: list[str]) -> str:
        ...  # returns session_id

    def say(self, session_id: str, utterance: str, posterior: float,
            final: bool = False) -> str:
        ...  # returns message_id

    def done(self, session_id: str, posterior: float) -> str:
        ...  # returns message_id

    def close_convergence(self, session_id: str) -> dict:
        ...  # returns {"mpc": float, "gar": float, "scr": float}
```
...
| `PanelBus` shared in-process object | SNP-CE owns one `PanelBus` per `session_id` |
| `Orchestrator` calls buses directly | Orchestrator replaced by skill method calls |
| `L9BaseSkillSupport` / `L9IESkillSupport` / `L9SNPSkillSupport` | Deleted; replaced by `IESkill` / `SNPSkill` over `L9` + `Episode` |
