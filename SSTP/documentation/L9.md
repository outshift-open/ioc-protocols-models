...
- `type=cip-repair`: the structured repair request emitted by a listener
  who raises a `contingency`. Carries the `target_message_id` being
  repaired and the `repair_reason` (`grounding_failure`,
  `scope_mismatch`, or `ungroundable_novelty`). Travels alongside the
  `type=utterance` part on a `contingency` message.
