...
**① intent** — Agent A opens the episode.  The `intent` carries the
...
**②–③ exchange** — Both agents contribute turns.  Each `exchange`
...
**① intent** — The `intent` is the routing message.  CF-A reads the
...
**⑥ commit:ready** — Agent B's readiness signal passes through both CE
chains and is delivered to Agent A.

**⑦ commit:converged** — Agent A closes the episode.  Both CEs tear
down per-episode state.  Both nodes delete their `EpisodeRoute` for
this episode.

**⑧ knowledge** — The `knowledge` message is topic-routed exactly like
