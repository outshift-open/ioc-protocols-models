...
in particular the migration of hcpanel's ``panel_bus.py`` /
``agent_bus.py`` onto the pydantic-based ``builder.py``
(``CIPMessageBuilder``/``SIEPMessageBuilder``) — has something
...
legacy flat-header adapter implementation with fixed
``message_id``/``timestamp_ms`` inputs (so output is fully deterministic).
If the builder vocabulary tables change intentionally, regenerate the
...
from SSTP.subprotocol.cip.src.builder import build_l9_header, _KIND_BY_EVENT_TYPE  # noqa: E402
from SSTP.subprotocol.siep.src.builder import build_snp_l9_header, NegotiationOperation  # noqa: E402
