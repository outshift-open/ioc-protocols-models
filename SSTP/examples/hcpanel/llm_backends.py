    ...
    def _instruction_for_task(self, task: str) -> str:
        instructions = {
            ...
            "debate_accept_or_counter": (
                ...
                "Payload: agent_id (str), role (str), my_taskwork_rationale (str — your independent prior assessment), "
                "my_supporting_evidence (list[str]), my_confidence (float — your own belief strength), "
                "proposal_concept (str), proposal_confidence (float — coordinator's stated confidence), "
                "proposal_rationale (str), proposal_evidence (list[str]), proposal_addresses_evidence (list[str]), "
                "task_goal (str), session_objective (str), round (int — 1 = first, 2+ = subsequent round), "
                ...
                "If countering, set counter_confidence to reflect YOUR actual belief strength (0..1) based on "
                "the clinical evidence in my_supporting_evidence and my_taskwork_rationale — do NOT default to 0.5. "
                ...
                "counter_confidence (float 0..1 reflecting your actual conviction — only if counter), "
                "rationale (str — grounds for your decision, referencing specific evidence), "
                "supporting_evidence (list[str]), "
                "thought_summary (str — one sentence)."
            ),
            "debate_pivot_synthesis": (
                "You are the panel coordinator pivoting your position after receiving counter-proposals. "
                "Payload: original_position ({likely_cause, confidence, rationale, supporting_evidence, addresses_evidence}), "
                "counter_proposals (list of {agent_id, likely_cause, confidence, rationale, supporting_evidence}), "
                "accept_positions (list — positions of agents who accepted), task_goal (str). "
                ...
                "rationale (str — why the pivot is warranted, engaging with each counter), "
                "supporting_evidence (list[str]), "
                "addresses_evidence (list[str] — explicit engagement with each counter-proposal's evidence), "
