    ...
    def _instruction_for_task(self, task: str) -> str:
        instructions = {
            ...
            "debate_accept_or_counter": (
                ...
                "my_supporting_evidence (list[str]), proposal_concept (str), proposal_confidence (float), "
                "proposal_rationale (str), proposal_evidence (list[str]), proposal_addresses_evidence (list[str]), "
                "task_goal (str), session_objective (str), "
                "controller_preempts_objection (str, optional — controller's anticipated rebuttal of your objection), "
                "high_derailment_risk (bool, optional). "
                "Compare the proposal against your own taskwork analysis. "
                "Accept only if the proposal genuinely accounts for your key evidence. "
                "If countering, state your alternative concept and confidence. "
                "Return JSON: decision ('accept' or 'counter'), "
                "counter_concept (str — only if counter), counter_confidence (float — only if counter), "
                "rationale (str — grounds for your decision), supporting_evidence (list[str]), "
                "thought_summary (str — one sentence)."
            ),
