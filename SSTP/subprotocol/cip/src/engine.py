...
CIPEngineConfig carries all domain-specific content as injectable data:
...
CIPEngine contains all the logic; zero domain strings are hard-coded.

The application defines a config constant and passes it at construction::

    MYAPP_IE_CONFIG = CIPEngineConfig(
        derailment_causes={...},
        intent_keywords={...},
        ...
    )
    engine = CIPEngine(config=MYAPP_IE_CONFIG, llm_client=llm)
"""
...
@dataclass
class CIPEngineConfig:
    """All domain-specific content for a CIPEngine instance.
...
class CIPEngine:
    """Domain-agnostic Contingency Interaction Protocol engine.
    ...
    domain-specific strings come from CIPEngineConfig.
...
IEEngineConfig = CIPEngineConfig
IEEngine = CIPEngine

__all__ = ["CIPEngineConfig", "CIPEngine", "IEEngineConfig", "IEEngine"]
