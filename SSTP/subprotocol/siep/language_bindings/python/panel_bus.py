...
"""
sstp/siep/panel_bus.py — Domain-agnostic SIEP + IE dual-protocol panel negotiation bus.
...
Every pairwise exchange emits simultaneously:
  1. An SIEP L9 header (build_snp_l9_header) → snp_trace
...
from SSTP.subprotocol.siep.language_bindings.python.negotiate import (
...
try:
    from SSTP.subprotocol.cip.language_bindings.python.l9 import build_l9_header
    from SSTP.subprotocol.cip.language_bindings.python.message import get_part as _get_part
except ImportError:
    build_l9_header = None  # type: ignore
    _get_part = None        # type: ignore
from SSTP.subprotocol.siep.language_bindings.python.epistemic import (
    SpeechAct, EpistemicState, BeliefStatus,
    make_epistemic_block, infer_snp_epistemic,
)
from SSTP.subprotocol.siep.language_bindings.python.epistemic.stores import (
    AgentBeliefStore,
    ArgumentOutcome,
    BeliefRevision,
    CommonGround,
...
if TYPE_CHECKING:
    from SSTP.subprotocol.cip.language_bindings.python.agent_bus import AgentBus
    from SSTP.subprotocol.cip.language_bindings.python.tom import TheoryOfMindEngineBase
    from SSTP.subprotocol.siep.language_bindings.python.epistemic.stores import PeerInteractionStore
...
class PanelBus:
    ...
    def negotiate_process(
        ...
        """
        from SSTP.subprotocol.siep.language_bindings.python.epistemic.stores import TeamProcessAgreement, RoleAssignment
        episode_id = self._episode_id()
        assignments = [
            RoleAssignment(
    ...
    def _verify_grounding_bilateral(
        ...
        if forced_accept:
            ...
            from SSTP.subprotocol.cip.language_bindings.python.grounding import contingency_check as _contingency_check, _get_concept_ids
        ...
        if not is_deliberation_pass and not forced_accept:
            ...
            # For SIEP utterances (machine-generated position strings), genuine content
...
class StarNegotiation:
    """Star-topology (hub-and-spoke) SIEP negotiation: controller ↔ N members."""
    ...
    def _emit_specialist_response(
        ...
        if _is_delib_pass:
            from SSTP.subprotocol.siep.language_bindings.python.epistemic.vocabulary import make_snp_epistemic_extension
    ...
    def run(
        ...
        for round_idx in range(max_rounds):
            ...
            if self.panel_bus.tom_engine is not None:
                ...
                for mid in member_ids:
                    ...
                    if hasattr(ctrl_agent, "predict_peer_response"):
                        ...
                        from SSTP.subprotocol.siep.language_bindings.python.epistemic.tom import predict_belief
            ...
            for member_id in member_ids:
                ...
                _resp_prop_id = _get_part(resp_snp, "siep").get("proposal_id") or resp_snp["message"]["id"]
        ...
        if self.panel_bus.convergence_store is not None:
            ...
            if (
                ...
                # Emit rule_update (kind=knowledge) on the panel SIEP trace
                _rule_utterance = (
    ...
    def run(
        ...
        for round_idx in range(max_rounds):
            ...
            for i in range(n):
                ...
                _neg_prop_id = _get_part(snp_neg, "siep").get("proposal_id") or snp_neg["message"]["id"]
