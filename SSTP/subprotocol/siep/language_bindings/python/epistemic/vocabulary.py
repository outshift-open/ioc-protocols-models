...
- infer_snp_epistemic(): deterministically infer (SpeechAct, EpistemicState) from SIEP
...
class TaskPhase(str, enum.Enum):
    ...
    INTERPERSONAL = "grounding"      # maps to GROUNDING (IE repair); SIEP callers use TEAM_PROCESS
...
def make_epistemic_block(
    *,
    ...
    SIEP sub-protocol extension (deferred_to) is set via make_snp_epistemic_extension()
...
def make_snp_epistemic_extension(
    ...
    *,
    ...
    """Merge SIEP sub-protocol extension fields into an existing epistemic block.

    SIEP extension fields (set only by the SIEP layer):
    ...
    Call after make_epistemic_block() when the block is for an SIEP message:
